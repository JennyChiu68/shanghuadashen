...
def query_page_image(site: str, title: str, user_agent: str) -> tuple[str | None, str | None]:
    params = {
        "action": "query",
        "prop": "pageimages",
        "format": "json",
...
def search_title(site: str, query: str, user_agent: str) -> str | None:
    params = {
        "action": "query",
        "list": "search",
        "format": "json",
...
def resolve_image(name: str, user_agent: str) -> tuple[str | None, str | None, str | None]:
    image_url, page_url = query_page_image("zh.wikipedia", name, user_agent)
    if image_url:
        return image_url, page_url, "zh.wikipedia"

    search_query = f"{name} 花"
    title = search_title("zh.wikipedia", search_query, user_agent)
    if title:
        image_url, page_url = query_page_image("zh.wikipedia", title, user_agent)
        if image_url:
            return image_url, page_url, "zh.wikipedia"

    title = search_title("en.wikipedia", name, user_agent)
    if title:
        image_url, page_url = query_page_image("en.wikipedia", title, user_agent)
        if image_url:
...
def main() -> None:
    ...
    parser.add_argument(
        "--user-agent",
        default="shanghuadashen-bot/1.0 (https://example.com/contact)",
    ...
    for record in flowers:
        if args.offline:
            continue
        try:
            image_url, page_url, source = resolve_image(record.name, args.user_agent)
